...
#define  _kNUMBER_OF_SCHEDULES                 _kMAX_SCHEDULE_PLOT
...
typedef struct Schedules
{
	...
	unsigned int  Running_Hr_Of_Each_Plot[_kMAX_SCHEDULE_PLOT+1]; //38 /**< each schedule run time */ 
	unsigned char Schedule_Status[_kMAX_SCHEDULE_PLOT+1];         //19 /**< what are the status of schedule */
	unsigned char Schedule_Num_List[_kMAX_SCHEDULE_PLOT+1];       //19 /**< save the list of schedule number list */
	unsigned char Scheduler_Already_Run[_kMAX_SCHEDULE_PLOT+1];   //19 /**< save the date when schedule was run */
	unsigned char Scheduler_Already_Run_Month[_kMAX_SCHEDULE_PLOT+1]; //19
	unsigned char Pump_On_In_Scheduler;  //1                            /**< pump on after schedule trigger */
	
	unsigned char Fertilizer_Tank_Number[_kMAX_SCHEDULE_PLOT*_kMAX_FERTILIZER_TANK + 1];   //55
	unsigned int  Fertilizer_Tank_On_After[_kMAX_SCHEDULE_PLOT*_kMAX_FERTILIZER_TANK + 1]; //55*2
	unsigned int  Tank_Off_Time_Duration[_kMAX_SCHEDULE_PLOT*_kMAX_FERTILIZER_TANK + 1];   //55*2
	...
	unsigned int  Dummy_Running_Hr_Of_Each_Plot[_kMAX_SCHEDULE_PLOT+1]; // 19*2 /* each schedule run time */
	unsigned char Dummy_Schedule_Status[_kMAX_SCHEDULE_PLOT+1];    //19
	unsigned char Dummy_Schedule_Num_List[_kMAX_SCHEDULE_PLOT+1];  //19
