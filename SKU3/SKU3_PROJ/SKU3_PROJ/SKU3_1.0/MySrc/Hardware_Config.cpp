...
volatile unsigned char rx_complete,rx_flag;
volatile unsigned int my_timer;
volatile unsigned char monitor_serial_start;

ISR(USART1_RX_vect,ISR_BLOCK)
{
	if(int_counter < sizeof(_gPub_Buff))
	{
		_gPub_Buff[int_counter++] = UDR1;// read the received data byte in temp
		rx_flag = _kSET;
	}
}
...
void Initialise_Hardware(void)
{
	...
	_KSERIAL_AT_INIT(9600);          /* serial for the AT commands */
	...
	_kGPIO_MODE(A3, _kINPUT_MODE);
	_kGPIO_MODE(A4, _kINPUT_MODE);
	_kGPIO_MODE(A5, _kINPUT_MODE);
...
// 	Timer0_Interrupt_Int();           /* used for 1 sec */
	...
	/*Serial.println("kikikikiki");
	while(1)
	{
		if(rx_complete == _kSET)
		{
			rx_complete = _kRESET;
			Serial.println((char *)_gPub_Buff);
...
// 	}
	...
	if(_gTimer_For_Three_Phase_Testing < 62)
	{
		_gTimer_For_Three_Phase_Testing++;
	}
	
	if(_gStart_Monitoring_Three_Phase < 75)
	{
		_gStart_Monitoring_Three_Phase++;
	}
...
ISR(TIMER2_COMPA_vect)
{
	//g_uTick++;
	//ADC_Operation();
	static unsigned char timer_int;
	if(rx_flag)
	{
		my_timer++;
	}
	
	if(my_timer > Interrupt_Time_Out && rx_flag == _kSET)
	{
		rx_complete = _kSET;
		rx_flag = _kRESET;
		int_counter  = 0;
...
char Send_AT_Command(char* ATcommand, char* expected_answer, char* expected_answer2,unsigned int timeout,unsigned char retry)
{
	...
	while(command_conter < retry)
	{
		...
		do
		{
			...
			{
				if (strstr((char *)_gPub_Buff, expected_answer) != NULL || strstr((char *)_gPub_Buff, expected_answer2) != NULL )
				{
					answer = 1;
					command_conter++;
				}
			...
			{
				if (strstr((char *)_gPub_Buff, expected_answer) != NULL)
				{
					answer = 1;
					command_conter++;
				}
	...
	{
		memset(temp_data,0,sizeof(temp_data));
		Serial.println("answer not matched");
		temp_data[0] = 0x1A;
		_kSERIAL_AT.print((char *)temp_data);
	}
	return answer;
