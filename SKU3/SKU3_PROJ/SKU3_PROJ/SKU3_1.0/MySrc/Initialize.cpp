...
volatile unsigned char _gStart_Monitoring_Three_Phase;
volatile unsigned char _gTimer_For_Three_Phase_Testing;
...
// 	 	_kGPIO_GET(_kB_PHASE_INPUT) != _kHIGH)
	...
	if(_gTimer_For_Three_Phase_Testing > 60)
	{
		if(_sRuble_Parameters.Debug_Mode == _kSET)
		{
...
// 			_kGPIO_GET(_kB_PHASE_INPUT) != _kHIGH)
			if(_kGPIO_GET(_kPHASE_DETECTION))
			{
				if(_sRuble_Parameters.Debug_Mode == _kSET)
				{
					_kSERAIL_MON_WRITE("phase is not there 1 ");
				}
				return 1;
			}
			if(_sRuble_Parameters.Debug_Mode == _kSET)
			{
				_kSERAIL_MON_WRITE("phase is there 1 ");
			}
			return 0;
		}
		_kSERAIL_MON_WRITE("phase is there 2 ");
		return 1;
	}
}
