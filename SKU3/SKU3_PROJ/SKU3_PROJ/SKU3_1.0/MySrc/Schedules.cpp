...
unsigned char Return_Next_Day_Date(void)
{
	...
	if(month== _eJAN || month== _eMARCH || month== _eMAY || month== _eJUL || month== _eAUG || month== _eOCT || month== _eDEC)
	{
		if(date > 30)
		{
			return 1;
	...
	if(month== _eAPR || month== _eJUN || month== _eSEP || month== _eNOV)
	{
		if(date > 29)
		{
			return 1;
	...
	if(month == _eFEB)    /* calculation for feb */
	{
		if(year%4) /* 28 days */
		{
			if(date > 27)
	...
	/* jan(1) march(3) may(5) july(7) aug(8) oct(10) dec(12) */
...
unsigned char Next_Day_Calculate(unsigned char date,unsigned char month)
{
	...
	switch(month)
	{
		...
		case _eJUN:
		{
			...
			{
				date = date - 30;
				if(( _sRuble_Parameters.Update_Local_Date[0] >= date   && c_month == _eJUL) ||
