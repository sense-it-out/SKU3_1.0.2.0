...
void Read_Switch_Status(void)
{
	...
	{
		if(time_count > 3)     /* if count will 50 (which is 50ms) then only this will execute */
		{
			switch(key_press)
			{
...
void Run_Operation(void)
{
	...
	if(_gStart_Monitoring_Three_Phase < 70)
	{
		return;
	}
...
void Publish_Data(void)
{
	...
	if(_sRuble_Parameters.Publish_Topic & _ePUB_RUBLE_FAULTS)          /* if the topic is ruble faults */
	{
		...
		sprintf((char *)_gRecvd_Data,"SICCA/CODES/%s",_sRuble_Parameters.Ruble_ID);
		
		
		
		while(retry > 0)
		{
			if(_kMQTT_PUB((unsigned char *)_gRecvd_Data,(unsigned char *)pub_data))   /* publish data to the server */
			{
				break;
			}
			retry--;
	...
	if(_sRuble_Parameters.Publish_Topic & _ePUB_TIMER_FEEDBACK)   /* if the topic Timer feedback */
	{
		...
		Transmit_Buff.prettyPrintTo((char *)pub_data, sizeof(pub_data)); 
		
		memset((char *)_gRecvd_Data,0,sizeof(_gRecvd_Data));
		                                          /* copy the JSON formatted data into the publish buffer */
        
		sprintf((char *)_gRecvd_Data, "SICCA/TIMER/FEEDBACK/%s",_sRuble_Parameters.Ruble_ID);
	...
	if(_sRuble_Parameters.Publish_Topic & _ePUB_LIRA_NODE_DATA)     /* if the topic lira node data */
	{
		...
		if(_sRuble_Parameters.Debug_Mode == _kSET)
		{
			_kSERIAL_MON_WRITE((char *)pub_data);
		}
		/* copy the JSON formatted data into the publish buffer */

		memset((char *)_gRecvd_Data,0,sizeof(_gRecvd_Data));
		sprintf((char *)_gRecvd_Data,"SICCA/SENSOR/DATA/%s","000000"); /* copy the topic including id */
		while(retry > 0)
		{
			if(_kMQTT_PUB((unsigned char *)_gRecvd_Data,(unsigned char *)pub_data))   /* publish data to the server */
			{
	...
	if(_sRuble_Parameters.Publish_Topic & _ePUB_HARDWARE_FAULTS)     /* if the topic is hardware fault */
	{
		...
		sprintf((char *)_gRecvd_Data,"SICCA/HARDWARE/FAULTS/%s",_sRuble_Parameters.Ruble_ID);
		while(retry > 0)
		{
			if(_kMQTT_PUB((unsigned char *)_gRecvd_Data,(unsigned char *)pub_data))   /* publish data to the server */
			{
	...
	if(_sRuble_Parameters.Publish_Topic & _eONE_TOUCH_FEEDBACK)
	{
		...
		memset((char *)_gRecvd_Data,0,sizeof(_gRecvd_Data));
		sprintf((char *)_gRecvd_Data,"SICCA/DEVICE/FEEDBACK/%s",_sRuble_Parameters.Ruble_ID);
		while(retry > 0)
		{
			if(_kMQTT_PUB((unsigned char *)_gRecvd_Data,(unsigned char *)pub_data))   /* publish data to the server */
			{
...
// 		Transmit_Buff[F("b_phase")] = _kGPIO_GET(_kB_PHASE_INPUT);
		...
		sprintf((char *)_gRecvd_Data ,"SICCA/CONTROLLER/DATA/%s",(const char *)_sRuble_Parameters.Ruble_ID);
		
		//SICCA/CONTROLLER/DATA/
...
void Dummy_Data_Store(void)
{
	...
	for(i=0; i< _kNUMBER_OF_SCHEDULES+1; i++)
	{
